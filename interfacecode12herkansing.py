    ...
    if circulation_file:
        if connexxion_file:
            with st.spinner("Please wait..."):
                ...
                for i in range(1,len(data_planning)):
                    eindtijd = data_planning['eindtijd'][i-1]
                    starttijd = data_planning['starttijd'][i]
                    bl = data_planning['buslijn'][i]
                    if eindtijd > starttijd and data_planning['omloop nummer'][i]==data_planning['omloop nummer'][i-1]:
                        No_mutiple_actions_simultaneously+=1
                
                wrong_startlocation=0
                for i in range(1,len(data_planning)):
                    eindlocatie = data_planning['eindlocatie'][i-1]
                    startlocatie = data_planning['startlocatie'][i]
                ...
                with col1:
                    ...
                    data_table=[['Idletime',penalty_idle],['Material_trip',penalty_mat],['Exceed time',penalty_tijd],
                                ['Not according to service trip',penalty_dienst],  ['charging too short',penalty_opladen],['Below safety margin',penalty_below_safety_margin],['Mutiple actions',No_mutiple_actions_simultaneously],['Wrong startlocation',wrong_startlocation]]
